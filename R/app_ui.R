...
# ===================================================================
app_ui <- function() {
  # Wrap the entire UI in a tagList to correctly add head elements
  tagList(
    # --- HEAD ELEMENTS ARE NOW AT THE TOP LEVEL ---
    tags$head(
      # 1. Favicon for the browser tab
      tags$link(rel = "icon", href = "LogoNobg.png", type = "image/png"),
    ...
    navbarPage(
      ...
      header = tagList(
        useShinyjs(),
        waiter::use_waiter()
        # The tags$head content was moved from here
      ),
